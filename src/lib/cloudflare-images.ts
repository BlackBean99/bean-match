...
const discoveredAccountIdCache = { value: null as string | null, resolved: false };
...
async function getCloudflareAccountId() {
  ...
  const response = await fetch(`${cloudflareApiBaseUrl}/accounts`, {
    headers: {
      Authorization: `Bearer ${getCloudflareImagesToken()}`,
  ...
  if (!response.ok) {
    discoveredAccountIdCache.resolved = true;
    discoveredAccountIdCache.value = null;
    return null;
  }
  ...
  const accounts = payload.result?.map((account) => account.id).filter((id): id is string => Boolean(id)) ?? [];
  discoveredAccountIdCache.resolved = true;
  discoveredAccountIdCache.value = accounts.length === 1 ? accounts[0] : null;
  return discoveredAccountIdCache.value;
}
