...
- 운영자는 사용자 상세에서 프로필 열람 토큰을 발급하고 `/readonly/pool/{userId}` 에서 같은 성별을 제외한 소개 가능 후보를 보여 주며, 토큰으로 접속한 사용자는 최대 3명까지 관심을 제출할 수 있습니다. `/users` 목록의 호감표시 링크는 `/offer/pool/{userId}?token={token}` 형태의 직접 진입 링크를 복사합니다.
