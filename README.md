...
- 참가자는 기존 데이터 ID와 이름으로 `/onboarding`에 진입하며, 자동 노출 풀 참여 여부와 알림 수신 여부를 직접 선택합니다.
- 참가자 개인 풀 URL은 `/pool/{userId}` 입니다. 신규 가입자는 여기서 최대 3명의 관심을 제출하고, 기존 회원은 새 멤버 알림에서 관심을 남깁니다.
- 운영자는 사용자 상세에서 읽기 전용 토큰을 발급하고 `/readonly/pool/{userId}` 에서 같은 성별을 제외한 소개 가능 후보만 읽기 전용으로 열람하게 할 수 있습니다.
- 운영자는 `/rounds` 에서 자동 노출 큐, 관심 현황, 소개 후보 승인/반려/전환을 관리합니다.
