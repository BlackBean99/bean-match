...
- 운영자는 사용자 상세에서 프로필 열람 토큰을 발급하고 `/readonly/pool/{userId}` 에서 같은 성별을 제외한 소개 가능 후보를 보여 주며, 토큰으로 접속한 사용자는 최대 3명까지 관심을 제출할 수 있습니다. 온보딩 접근 링크는 복사 시 토큰이 없으면 자동 발급해 함께 전달합니다.
