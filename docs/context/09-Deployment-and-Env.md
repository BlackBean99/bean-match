...
- `CLOUDFLARE_API_TOKEN` (Cloudflare Worker 배포용 비밀)
...
- 사진 조회는 `/api/photos/{photoId}`를 통해 Supabase Storage 원본을 서버 프록시합니다.
...
- 그 workflow는 배포 전에 Worker runtime secret payload를 함께 생성합니다. 운영 runtime에는 `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_URL` 또는 `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_STORAGE_BUCKET`, `NOTION_TOKEN`, `NOTION_API_VERSION`, `NOTION_MAIN_DATA_SOURCE_ID` 또는 `NOTION_USERS_DATABASE_ID`, `NOTION_INVITOR_DATA_SOURCE_ID`, `NOTION_MATCHING_HISTORY_DATA_SOURCE_ID`, `OPS_AUTH_SECRET`, `OPS_AUTH_ACCOUNTS_JSON`, `NOTION_SYNC_WORKFLOW_TOKEN` 를 주입합니다. `DATABASE_URL` 은 Worker runtime secret로 주입하지 않습니다.
...
- 오래된 source URL을 Supabase Storage reference로 바꾸려면 `NOTION_TOKEN`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` 를 설정한 뒤 `npm run sync:notion -- --write` 를 다시 실행합니다.
