...
Notion photo sync is treated as part of the write path. If a Notion image cannot be downloaded or uploaded to Supabase Storage, that page is reported as a sync error instead of silently falling back to a temporary Cloudflare or Notion URL.
...
If older rows still have source URLs instead of Supabase Storage references, re-run the write sync after updating the photo pipeline so the rows are rewritten into Supabase Storage.
