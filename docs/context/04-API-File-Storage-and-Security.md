...
- Notion에서 가져온 파일도 Supabase Storage `beanmatch-image-storage` 버킷에 저장한다.
- Notion sync는 원본 이미지를 그대로 저장하지 않고, 회전 보정 후 WebP로 변환한 원본본(large)과 썸네일(thumb)을 함께 저장한다.
- `file_url`에는 원본 WebP 참조를, `file_path`에는 썸네일 WebP 참조를 기록한다.
...
- `original_file_name`
- `stored_file_name`
- `file_path`: 썸네일용 `supabase-storage:{bucket}/{path}` 참조
- `file_url`: 원본용 `supabase-storage:{bucket}/{path}` 참조
- `mime_type`
- `file_size_bytes`
- `width_px`
- `height_px`
...
### 4.4 디렉터리 구조 권장
- 애플리케이션은 별도 업로드 디렉터리를 만들지 않는다. 이미지 원본 저장은 Supabase Storage가 담당하고, 화면 노출은 `/api/photos/{photoId}` 서버 프록시가 담당한다.

### 4.5 삭제 정책
- DB soft delete + 파일 비동기 삭제 또는 보존 정책 선택
- 대표 사진 삭제 시 다른 사진을 대표로 승격하거나 `main_photo_id = null` 처리
- 대표 사진 변경 시 해당 사진을 1번으로 승격하고 나머지 사진 순서를 다시 정렬한다.
- 화면 표시 순서는 `is_main` 우선, `sort_order` 보조 규칙을 따른다.
